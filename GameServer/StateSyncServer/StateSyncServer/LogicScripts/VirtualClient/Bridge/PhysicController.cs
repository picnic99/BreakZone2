...
public class PhysicAction
{
    ...
    public bool IsEnd
    {
        get
        {
            return CommonUtils.GetCurTimeStamp() >= this.endTime;
        }
    }
    ...
    public PhysicAction(Vector3 totalOffset, float time, PhysicActionType type)
    {
        ...
        this.time = time;
        this.type = type;
        this.endTime = CommonUtils.GetCurTimeStamp() + (int)(this.time * 1000);
    }
}
...
public class PhysicController
{
    ...
    public PhysicController(Character character)
    {
        ...
        //TimeManager.GetInstance().AddLoopTime(this,0,Global.FixedFrameTimeMS,OnUpdate);
    }
    ...
    {
        Move(Vector3.UnitY * height, durationTime, true, false);
    }
...
/*            if (actionOffset.Y >= GravityOffset)
        ...
        else
        {
            ...
            if (lastIsGround == false)
            {
                if (IsJump)
                {
                    ...
                    CommonUtils.Logout(character.PlayerId + " 跳跃结束！");
                }
        ...
        //滞空移动
        if (CanControl && !isGround && actionOffset.Length() > 0 && IsJump)
        {
        ...
        //cc.Move(totalMove);
        character.Trans.Translate(totalMove);
        if (!isGround)
        {
            CommonUtils.Logout(character.PlayerId + " physic actionOffset = " + actionOffset.ToString());
            CommonUtils.Logout(character.PlayerId + " physic dir = " + dir.ToString());
            CommonUtils.Logout(character.PlayerId + " physic GravityOffset = " + GravityOffset.ToString());
            CommonUtils.Logout(character.PlayerId + " physic Pos = " + character.Trans.Position.ToString());
        }
    }
